...
class SolutionArray {
        ...
        Cell anotherEnd (Cell end1) {
                ...
                if (hv == 0) {
                        end2x = end1x;
                        if ((Gui.FIXBOXYMAX-end1y) >= (end1y-Gui.FIXBOXYMIN))
                                end2y = end1y + 1 + rand.nextInt(Gui.FIXBOXYMAX-end1y);
                        else end2y = end1y - 1 - rand.nextInt(end1y-Gui.FIXBOXYMIN);
                }
                else {
                        end2y = end1y;
                        if ((Gui.FIXBOXXMAX-end1x) >= (end1x-Gui.FIXBOXXMIN))
                                end2x = end1x + 1 + rand.nextInt(Gui.FIXBOXXMAX-end1x);
                        else end2x = end1x - 1 - rand.nextInt(end1x-Gui.FIXBOXXMIN);                                
                }
        ...
        SolutionArray (int n) {
                ...
                for (int i = 0; i < n; ++i) {
                        ...
                        if (i == 0) {
                                end1x = Gui.FIXBOXXMIN + rand.nextInt(Gui.FIXBOXXMAX-Gui.FIXBOXXMIN+1);
                                end1y = Gui.FIXBOXYMIN + rand.nextInt(Gui.FIXBOXYMAX-Gui.FIXBOXYMIN+1);
