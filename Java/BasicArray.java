        ...
        BasicArray (Magnet[] sArray) {
                ...
                for (int i = 0; i < sArray.length; ++i) {
                        ...
                        if (m.getNorth().x == m.getSouth().x) {
                                m = m.shift(1 + rand.nextInt(Gui.XCELLMAX-1) - m.getNorth().x,
                                                        1 + rand.nextInt(Gui.YCELLMAX-1-Math.abs(m.getNorth().y-m.getSouth().y)) - Math.min(m.getNorth().y, m.getSouth().y));
                        }
                        else {
                                m = m.shift(1 + rand.nextInt(Gui.XCELLMAX-1-Math.abs(m.getNorth().x-m.getSouth().x)) - Math.min(m.getNorth().x, m.getSouth().x), 
                                                        1 + rand.nextInt(Gui.YCELLMAX-1) - m.getNorth().y);
                        }
